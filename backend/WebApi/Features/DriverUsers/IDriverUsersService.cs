...
public interface IDriverUsersService
{
    ...
    Task<List<PointsModel>> GetPointsByUserIdAsync(string userId, int? orgId);
    Task<List<PointsModel>> GetPointsByIdAsync(int driverId, int? orgId, string? sponsorUserId);
